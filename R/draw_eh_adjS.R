draw.eh <- function(output,
                    ...
                    adj_panel_width = 0.10,          # narrow panel
  ...
  adj_all <- NULL
  if (!is.null(adj_file)) {
    adj_all <- utils::read.table(adj_file, header = FALSE, sep = "\t", stringsAsFactors = FALSE)
    if (ncol(adj_all) < 4) stop("adj_file must have >=4 columns: ancestor, chr, pos, score")
  ...
  for (ID in c(chrRange)) {
    ...
    subsetChr1 <- subset(
      alignments, chr == ID,
      select = c(chr, start, end, tarChr, tarSt, tarEnd, orient, tar, text_size2)
    )

    if (nrow(subsetChr1) == 0) {
      warning(paste0("No EH alignments found for chr ", ID, " (skipping)."))
      next
    }
    ...
    p_main <- ggplot2::ggplot() +
      ...
      ggplot2::geom_rect(
        data = subsetChr1,
        ggplot2::aes(xmin = 0, xmax = 0.5, ymin = ymin_gen, ymax = ymax_gen),
        linewidth = 0.3,
        color = "black",
        fill = NA
      ) +
      ...
      ggplot2::scale_y_reverse(limits = c(ymax_gen, ymin_gen), expand = c(0, 0)) +
      ggplot2::theme(
    ...
    if (!is.null(adj_all)) {
      ...
      if (nrow(adj_chr) > 0) {
        ...
        p_adj <- ggplot2::ggplot(adj_chr, ggplot2::aes(x = score_t, y = pos)) +
          ggplot2::geom_vline(
          ...
          ggplot2::theme(
            ...
            axis.text.x = ggplot2::element_blank(),
            axis.ticks.x = ggplot2::element_line(linewidth = 0.2),
    ...
    if (!is.null(p_adj)) {
      if (requireNamespace("patchwork", quietly = TRUE)) {
        final_plot <- p_main + p_adj +
          patchwork::plot_layout(widths = c(1, adj_panel_width))
      } else if (requireNamespace("gridExtra", quietly = TRUE)) {
        final_plot <- gridExtra::arrangeGrob(
          p_main, p_adj, nrow = 1, widths = c(1, adj_panel_width)
        )
      }
    }
